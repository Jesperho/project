...
def get_airport_iso(icao: str) -> str:
    sql = "SELECT iso_country FROM country WHERE iso_country IN (SELECT iso_country FROM airport WHERE ident=%s)"
    cursor = db.cursor()
    cursor.execute(sql, (icao,))
    country_iso = cursor.fetchone()
    return country_iso[0]
...
def move_player(playerid: str, destination_icao: str, weather_tuple: tuple) -> bool:
    ...
    distance = calculate_distance(
        get_airport_coordinates(current_location_icao),
        get_airport_coordinates(destination_icao),
    )
...
def player_wants_to_move(playerid: str) -> bool:
    while True:
        user_choice = (
            input(f"{get_player_name(playerid)}, do you want to fly there? (y/n): ")
            .strip()
            .lower()
        )
        if user_choice == "y":
...
def start_game():
    ...
    while True:  # Main loop
        ...
        for player in players:
            ...
            print(
                f"{get_player_name(player)} has a CO2 budget of {get_co2_budget(player)} now."
            )
            input()
            # Players get notified of their current location and destination
            next_destination = get_players_next_location(player)
            distance = calculate_distance(
                get_airport_coordinates(get_player_location(player)),
                get_airport_coordinates(next_destination),
            )
            print(
                f"The next destination for {get_player_name(player)} is {get_airport_country_name(next_destination)}, {get_airport_name(next_destination)}.\n"
            )
            print(
                f"The distance between current location and {get_airport_name(next_destination)} is {distance}."
            )

            # Get a weather condition
            weather_tuple = (
                get_random_weather_condition()
            )  # Get a random weather tuple where (name, description)
            ...
            if bad_weather_prevents_a_flight(distance, weather_tuple):
                print(
                    f"The weather is not suitable for a flight. {get_player_name(player)} waits for one move."
                )
                break

            print(
                f"The cost to move there condidering current weather conditions is {calculate_co2_expenditure(distance, weather_tuple)}."
            )
            ...
            if move_player(player, next_destination, weather_tuple):
                ...
                print(
                    f"{get_player_name(player)} reached {get_airport_country_name(next_destination)}!"
                )
            ...
            # If not enough co2 for the move, let the player know
                ...
                print(
                    f"{get_player_name(player)} does not have enough CO2 budget. Wait for the next roll."
                )
            input()
